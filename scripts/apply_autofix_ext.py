...
REPORT_JSON = ROOT / 'audit_out' / f'autofix_apply_report_ext_{datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")}.json'
BACKUP_SUFFIX = f'.bak_autofix_ext_{datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")}'
...
# group proposals by file and by file_line priority
by_file = {}
for p in proposals:
    file = p.get('file')
    if not file:
    ...
    for p in items_sorted:
        ...
        if file_line:
            try:
                ...
                row_idx = None
        # If have explicit row, attempt to patch that row
        target_idxs = []
        if row_idx is not None and 0 <= row_idx < len(lines):
            target_idxs = [row_idx]
        else:
            # fallback: search for a row that contains the suggestion text or row_text
        ...
        if not target_idxs:
            ...
            continue
        # try to parse and fill
        for ti in target_idxs:
            try:
                row = next(csv.reader([lines[ti]]))
            except Exception:
                continue
            # pos_id index is 14 if header same as others; guard
            if len(row) <= 14:
                # pad if necessary
                row += [''] * (15 - len(row))
            ...
            changed = True
            # do not try multiple proposals for same row
            break
    if changed:
        filep.write_text('\n'.join(lines) + '\n', encoding='utf-8')
